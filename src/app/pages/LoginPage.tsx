...
export function LoginPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setErrors({
        form: submitError instanceof Error ? submitError.message : 'Login failed.',
      });
    }
  }

  return (
    <AuthLayout title="Welcome back" subtitle="Jump back in and keep your play records tidy.">
      <form onSubmit={handleSubmit} className="space-y-4" noValidate>
        ...
        <div className="space-y-2">
          <Label htmlFor="login-email" className="text-stone-800 dark:text-stone-200">
            Email
          </Label>
          <Input
            id="login-email"
        ...
        <div className="space-y-2">
          <Label htmlFor="login-password" className="text-stone-800 dark:text-stone-200">
            Password
          </Label>
          <PasswordInput
            id="login-password"
        ...
        <div className="space-y-2 pt-2 text-center text-sm text-stone-500 dark:text-stone-400">
          <p>
            New around here?{' '}
            <Link to="/register" className="font-bold text-violet-600 hover:underline dark:text-violet-300">
              Create an account
            </Link>
          </p>
