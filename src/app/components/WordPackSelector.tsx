...
export function WordPackSelector({ currentPack, isHost, onChange }: WordPackSelectorProps) {
  ...
  const handleImportSample = () => {
    const sample = ['rumah', 'mobil', 'kucing', 'buku', 'pohon', 'matahari'];
    setCustomWords((prev) => [...new Set([...prev, ...sample])]);
  };
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2">
        ...
        <label className="text-sm font-semibold text-stone-800 dark:text-stone-200">Word Pack</label>
      </div>
      ...
      <div className="grid grid-cols-2 gap-1.5">
        {WORD_PACKS.map((pack) => (
          <button
            key={pack.id}
            disabled={!isHost}
            onClick={() => { onChange(pack.id); setShowCustom(false); }}
            className={cn(
          ...
          >
            <span className="text-lg" aria-hidden>{pack.emoji}</span>
            <div className="min-w-0">
              <p className="text-xs font-bold text-stone-800 dark:text-stone-200 truncate">{pack.name}</p>
              <p className="text-xs text-stone-400 dark:text-stone-500">{pack.words.length} words</p>
            </div>
          </button>
        ))}
        ...
        >
          ...
          <div className="min-w-0">
            <p className="text-xs font-bold text-stone-800 dark:text-stone-200">Custom</p>
            <p className="text-xs text-stone-400 dark:text-stone-500">{customWords.length} words</p>
          </div>
        </button>
      </div>
      ...
      {showCustom && isHost && (
        <div className="space-y-2 p-3 rounded-xl border-2 border-violet-300 dark:border-violet-700 bg-violet-50 dark:bg-violet-900/10">
          <div className="flex items-center justify-between">
            <p className="text-xs font-bold text-violet-700 dark:text-violet-300">Custom Words ({customWords.length})</p>
            <button
